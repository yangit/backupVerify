...
const integrityCheck = async ({
  ...
  try {
    ...
    const { durationSec: pruningDuration } = await runCommand({
      failOnExitCode: true,
      command: `./${duplicacyBinary} prune -storage ${storage} -keep 0:360 -keep 30:180 -keep 7:30 -keep 1:7 -threads 5 2>&1 | tee ~/duplicacy.log`,
    });
  ...
  try {
    ...
    const { checkedRevisions, durationSec: durationQuickCheck } = parser.checkQuick(
      await runCommand({
        failOnExitCode: true,
        command: `./${duplicacyBinary} check -storage ${storage} -threads 5 2>&1 | tee ~/duplicacy.log`,
      }),
    );
  ...
  try {
    ...
    const { filesOk, durationSec: filesOkDuration } = parser.checkFiles(
      await runCommand({
        failOnExitCode: true,
        command: `./${duplicacyBinary} check -storage ${storage} -r ${status.lastRevision.number} -files -threads 5 2>&1 | tee ~/duplicacy.log`,
      }),
    );
  ...
  try {
    ...
    const { chunksOk, durationSec: chunksOkDuration } = parser.checkChunks(
      await runCommand({
        failOnExitCode: true,
        command: `./${duplicacyBinary} check -storage ${storage} -chunks -threads 5 2>&1 | tee ~/duplicacy.log`,
      }),
    );
